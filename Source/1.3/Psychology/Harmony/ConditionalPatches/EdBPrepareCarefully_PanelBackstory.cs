...
namespace Psychology.Harmony
{
    ...
    public static class EdBPrepareCarefully_PanelBackstory_Patch
    {
        ...
        public static void Draw(EdB.PrepareCarefully.State state, float y)
        {
            ...
            {
                Rect buttonRect = new Rect(224f, y2 + 5f, 28f, 28f);
                Color oldColor = GUI.color;
                GUI.color = buttonRect.Contains(Event.current.mousePosition) ? PsychColor.ButtonLightColor : PsychColor.ButtonDarkColor;
                GUI.DrawTexture(buttonRect, PsychologyTexCommand.PsycheButton);
                GUI.color = oldColor;

...
public static class PawnObserver_ObserveSurroundingPatch
{
    ...
    public static IEnumerable<CodeInstruction> DesensitizeViaCorpse(IEnumerable<CodeInstruction> instrs)
    {
        ...
        {
            yield return itr;
            if (itr.opcode == OpCodes.Callvirt && itr.operand as MethodInfo == AccessTools.Method(typeof(IObservedThoughtGiver), nameof(IObservedThoughtGiver.GiveObservedThought), new Type[] { }))
            {

...
namespace Psychology.Harmony
{
    ...
    public static class InteractionWorker_RomanceAttempt_SelectionWeightPatch
    {
        ...
        public static void PsychologyException(ref float __result, Pawn initiator, Pawn recipient)
        {
            ...
            if (recipient.relations.OpinionOf(initiator) < PsychologyBase.RomanceThreshold() && !initiator.story.traits.HasTrait(TraitDefOfPsychology.Lecher))
            {
                __result = 0f;
                return;
            }
            ...
            float driveModifer = Mathf.Lerp(0f, 2f, (PsycheHelper.Comp(initiator).Sexuality.AdjustedSexDrive) + PsycheHelper.Comp(initiator).Sexuality.AdjustedRomanticDrive);
            ...
            __result = romanceChance * existingLovePartnerFactor * attractivenessFactor * opinionFactor * knownSexualityFactor * recipientLovePartnerFactor * driveModifer * PsychologyBase.RomanceChance();
            return;
        }
    }

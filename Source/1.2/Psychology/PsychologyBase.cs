...
namespace Psychology
{
    public class PsychologyBase : ModBase
    {
        ...
        private SettingHandle<bool> beatyOverrideEnabled; // v1.2
    ...
    static public bool EnablePerformanceTesting()
        ...
        public override void SettingsChanged()
        {
            ...
            beautyOverrideConfig = beatyOverrideEnabled.Value; //1.2
        ...
        public override void DefsLoaded()
        {
            if (ModIsActive)
            {
                ...
                // 1.2 configs
                beatyOverrideEnabled = Settings.GetHandle<bool>("BeatyOverrideEnabled", "BeatyOverrideEnabledTitle".Translate(), "BeatyOverrideEnabledToolip".Translate(), true);
        ...
        public override void MapLoaded(Map map)
        {
            if (ModIsActive && PsychologyBase.ActivateKinsey())
            {
                ...
                foreach (Pawn pawn in gayPawns)
                {
                    ...
                    if (PsycheHelper.PsychologyEnabled(pawn) && PsycheHelper.Comp(pawn).Sexuality.sexDrive < 0.05f)
                    {
                        PsycheHelper.Comp(pawn).Sexuality.sexDrive = Rand.Value * 0.05f;
                    }
                }
                /* UPP: Removes bisexual trait */
                ...
                foreach (Pawn pawn in gayPawns)
                {
                    ...
                    if (PsycheHelper.PsychologyEnabled(pawn) && PsycheHelper.Comp(pawn).Sexuality.kinseyRating < 5)
                    {
                        PsycheHelper.Comp(pawn).Sexuality.kinseyRating = Rand.RangeInclusive(2, 4);
                    }
                }
            }
        }
            ...
            if (currentTick % GenDate.TicksPerHour*2 == 0)
            {
                ...
                if(constituents.Count() > 0)
                {
                    ...
                    if (potentialConstituent != null && !potentialConstituent.Downed && !potentialConstituent.Drafted && potentialConstituent.health.summaryHealth.SummaryHealthPercent >= 1f && potentialConstituent.GetTimeAssignment() != TimeAssignmentDefOf.Work && activeMayors.Count() > 0)
                    {
                        ...
                        if (PsycheHelper.PsychologyEnabled(potentialConstituent) && Rand.Chance((1f - PsycheHelper.Comp(potentialConstituent).Psyche.GetPersonalityRating(PersonalityNodeDefOf.Independent)) / 5f) && (found != null || RCellFinder.TryFindGatheringSpot(mayor, GatheringDefOf.Party, out gather))

...
namespace Psychology
{
  class LordJob_Joinable_Election : LordJob_VoluntarilyJoinable
  {
    ...
    private void Finished()
    {
      ...
      Hediff_Mayor hediffMayor = (Hediff_Mayor)HediffMaker.MakeHediff(HediffDefOfPsychology.Mayor, newMayor);


      hediffMayor.worldTileElectedOn = map.Tile;
      hediffMayor.yearElected = GenLocalDate.Year(map);

      newMayor.health.AddHediff(hediffMayor);
      PsycheHelper.GameComp.Mayors.Add(mapTile, new Pair<Pawn, Hediff_Mayor>(newMayor, hediffMayor));
